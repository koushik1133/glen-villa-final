...
export async function POST(req: Request) {
  try {
    ...
    const { getSession: getSess, assertBrandAccess } = require("@/lib/auth/session");
    ...
    if (error instanceof Error) {
      return NextResponse.json({ ok: false, error: error.message }, { status: (error as any).status || 500 });
    }
