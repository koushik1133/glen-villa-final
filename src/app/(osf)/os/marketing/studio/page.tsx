...
export default async function StudioPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const back = selected ? `/os/marketing/studio?draft=${selected.id}` : "/os/marketing/studio";
    ...
    <>
      ...
      >
        {!selected ? (
          ...
          <Empty
            action={
              <Link href="/os/settings/integrations" className="btn-ghost">
      ...
      <div className="mt-3 flex items-center gap-2 border-t border-[--color-line] pt-3">
        {locked ? (
          ...
          <form action="/api/osf/marketing/publish" method="POST" className="flex flex-1 items-center gap-2">
            ...
            <input type="hidden" name="next" value={`/os/marketing/studio?draft=${draft.id}`} />
            <select
              name="status"
