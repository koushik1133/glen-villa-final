...
import { Card } from "@/components/osf/ui";
...
export function VoiceTester() {
  ...
  async function upload(blob: Blob) {
    ...
    try {
      ...
      const res = await fetch("/api/osf/whatsapp/test-voice", { method: "POST", body: form });
  ...
  return (
    <Card
      ...
      hint="Record here and it runs the exact path a WhatsApp voice note takes: download, voice transcription, then the sales agent. Works before Meta is connected."
    >
      ...
      {error && (
        <p className="mt-4 rounded-lg border border-[var(--color-danger)]/30 bg-[color-mix(in_oklab,var(--c-bad)_8%,transparent)] px-3 py-2 text-xs text-[var(--color-danger)]">
          {error}
        </p>
      )}
