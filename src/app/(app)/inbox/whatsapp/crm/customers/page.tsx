...
import { BOOKING_STATUS_LABELS, customers, humanise, type CustomerRow } from "@/lib/osf/crm";
    ...
    <>
      ...
      {rows.length === 0 ? (
        <Card>
          <Empty
            action={
              <Link href="/inbox/whatsapp/crm/pipeline" className="btn-ghost">
                Open the pipeline
              </Link>
            }
          >
...
function CustomerRowView({ row }: { row: CustomerRow }) {
  ...
  return (
    <tr className="row-hover">
      <td className="td">
        {row.leadId ? (
          <Link href={`/inbox/whatsapp/crm/leads/${row.leadId}`} className="font-medium hover:text-[var(--color-gold-300)]">
            {row.name}
          </Link>
        ) : (
