...
function filterHref(temp?: string, sent?: string): string {
  ...
  const q = p.toString();
  return q ? `/inbox/whatsapp/crm/rankings?${q}` : "/inbox/whatsapp/crm/rankings";
}
  ...
  return (
    <tr className="border-b border-[var(--color-line)]/60 last:border-0 hover:bg-[var(--color-raised)]/50">
      ...
      <td className="py-3 align-middle">
        <Link href={`/inbox/whatsapp/crm/leads/${c.id}`} className="font-medium hover:text-[var(--color-gold-300)]">
          {c.name ?? "Unnamed"}
        </Link>
