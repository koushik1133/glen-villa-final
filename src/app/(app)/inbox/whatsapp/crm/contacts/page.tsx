...
import { Card, Empty, PageHeader, SetupNotice, Stat, formatNumber, timeAgo } from "@/components/osf/ui";
import { contactDirectory, humanise, type ContactRow } from "@/lib/osf/crm";
import { gatedLoad } from "@/lib/osf/queries";

export const dynamic = "force-dynamic";

const BASE = "/inbox/whatsapp/crm/contacts";
  ...
  return (
    <>
      ...
      {all.length === 0 ? (
        <Card>
          <Empty>
            <code className="rounded bg-[var(--color-canvas)] px-1.5 py-0.5 text-xs">villa_contacts</code>{" "}
  ...
  return (
    <tr className="row-hover">
      <td className="td">
        ...
        {contact.notes && (
          <span className="mt-1 block max-w-md text-[11px] leading-relaxed text-[var(--color-faint)]">
            {contact.notes}
          </span>
        )}
      </td>
      <td className="td text-xs capitalize">{humanise(contact.contact_type)}</td>
      <td className="td text-xs">{contact.company ?? <span className="text-[var(--color-faint)]">—</span>}</td>
      <td className="td text-xs">
        {contact.leadCount === 0 ? (
          <span className="text-[var(--color-faint)]">None</span>
        ) : contact.leadId ? (
          <Link
            href={`/inbox/whatsapp/crm/leads/${contact.leadId}`}
      ...
      <td className="td whitespace-nowrap text-xs text-[var(--color-muted)]">{timeAgo(contact.first_seen_at)}</td>
