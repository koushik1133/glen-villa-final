...
export type LeadSentiment = (typeof SENTIMENTS)[number];
...
export interface RankedClient {
  ...
  /** Written by the agent on every turn; null until they have said something. */
  ...
  /** 1-based position in the full ranking, assigned after sorting. */
  rank: number;
}

export interface ClientRanking {
  clients: RankedClient[];
  /** Counts across the WHOLE book, not just the rows returned. */
  totals: { all: number; hot: number; warm: number; cold: number };
 ...
 */
export async function clientRanking(limit?: number): Promise<ClientRanking> {
  const { data } = await db()
    .from("villa_leads")
    .select(
      `id, name, phone, lead_score, lead_temperature, sentiment, pipeline_stage,
  ...
  const rows = (data ?? []) as unknown as Omit<RankedClient, "rank">[];

  const totals = { all: rows.length, hot: 0, warm: 0, cold: 0 };
  const sentiment = { positive: 0, neutral: 0, negative: 0, unknown: 0 };
  for (const r of rows) {
    if (r.lead_temperature === "hot") totals.hot += 1;
    else if (r.lead_temperature === "warm") totals.warm += 1;
    else totals.cold += 1;
