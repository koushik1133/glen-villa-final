...
import { activeProvider } from "./whatsapp/outbound";
...
function fillPlaceholders(body: string, params: string[]): string {
...
async function runBatch(broadcastId: string): Promise<SendResult> {
  ...
  for (const recipient of batch) {
    ...
    try {
      const params = renderVariables(broadcast.variables, lead as Lead);
      const { messageId } =
        provider === "evolution"
          ? await sendEvolutionText(recipient.phone, fillPlaceholders(template.body, params))
          : await sendTemplate(recipient.phone, template.name, template.language, params);
      await supabase
        .from("villa_broadcast_recipients")
