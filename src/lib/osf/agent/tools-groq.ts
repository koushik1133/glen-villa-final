 ...
 */
function stripPropertyDescriptions(schema: JsonSchemaObject): JsonSchemaObject {
  if (schema.type !== "object" || !schema.properties) return schema;
  const properties: Record<string, SchemaProperty> = {};
  for (const [key, prop] of Object.entries(schema.properties)) {
    const { description: _drop, ...rest } = prop as SchemaProperty & { description?: string };
    properties[key] = rest;
  }
  return { ...schema, properties };
}

export const GROQ_TOOLS: GroqTool[] = TOOLS.filter((t) =>
  GROQ_TOOL_ALLOWLIST.has(t.name),
).map((t) => ({
  type: "function",
  function: {
    name: t.name,
    description: t.description ?? "",
    parameters: stripPropertyDescriptions(
      allowNullOnOptionalFields(t.input_schema as unknown as JsonSchemaObject),
