...
import { DEFAULT_AVAILABILITY, HOLDS_SLOT } from "./types";
import { availabilityFor, saveAvailability, slots, zonedDate, zonedInstant } from "./engine";
import { uid } from "../ids";
...
function at(daysAhead: number, hour: number, tz: string): string {
...
function daysUntilWeekday(target: number, tz: string): number {
  for (let i = 1; i <= 7; i++) {
    const d = new Date(Date.now() + i * 86_400_000);
    const name = new Intl.DateTimeFormat("en-US", { timeZone: tz, weekday: "short" }).format(d);
    if (["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].indexOf(name) === target) return i;
  }
  return 7;
}
...
export const SEED_NOTE = "Sample booking — demonstration data, replace with real visits.";
 ...
 */
export function seedSiteVisitDesk(brandId: string, timezone?: string): number {
  const db = read();
  const now = Date.now();
  const upcoming = (db.appointments ?? []).some(
    (a) => a.brandId === brandId && new Date(a.startsAt).getTime() >= now,
  );
  if (upcoming) return 0;

  const tz = timezone || availabilityFor(brandId).timezone || DEFAULT_AVAILABILITY.timezone;
  ...
  // behave correctly.
  saveAvailability({ ...DEFAULT_AVAILABILITY, brandId, timezone: tz });

  const saturday = daysUntilWeekday(6, tz);
  const sunday = daysUntilWeekday(0, tz);
  const capacity = DEFAULT_AVAILABILITY.concurrentCapacity;

  const made: Appointment[] = [];
  let n = 0;
...
function sample(brandId: string, startsAt: string, name: string, i: number): Appointment {
  ...
  return {
    ...
    startsAt,
    durationMinutes: DEFAULT_AVAILABILITY.slotMinutes,
    status: "confirmed",
    channel: "walk_in",
    notes: SEED_NOTE,
    createdBy: "seed",
...
export function deskIsSeeded(brandId: string): boolean {
  const now = Date.now();
  const list = (read().appointments ?? []).filter(
    (a) => a.brandId === brandId && new Date(a.startsAt).getTime() >= now,
  );
  return list.length > 0 && list.every((a) => a.notes === SEED_NOTE);
}
