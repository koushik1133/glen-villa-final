...
export interface OnyxUnitType {
  position: 1 | 2 | 3 | 4 | 5 | 6 | 7;
  /** Built-up area in sq ft, exactly as printed. */
  sqFt: number;
  facing: "North" | "East" | "West" | "South";
  /** Only set where the drawn room schedule justifies it. */
  bhk?: string;
  rooms: Array<{ name: string; dimensions?: string }>;
  /** Public path to that unit's APARTMENT PLAN sheet. */
  planImage: string;
  /** Where the unit sits on the plate, per the key-plan inset on its sheet. */
  keyPlanNote?: string;
}
...
export const ONYX_PLATE_LAYOUT: Record<number, { col: 0 | 1 | 2; row: 0 | 1 | 2 }> = {
  2: { col: 0, row: 0 },
  3: { col: 1, row: 0 },
  4: { col: 2, row: 0 },
  1: { col: 0, row: 1 },
...
export const ONYX_UNIT_TYPES: readonly OnyxUnitType[] = [
  {
    position: 1,
    sqFt: 2594,
    facing: "North",
...
export const ONYX_UNIT_PLANS_OVERVIEW = "/showcase/onyx/apartment-plans-overview.webp";

/** Position on the plate for a flat number: "1204" → 4, "0101" → 1. */
export function onyxUnitPosition(unitNumber: string): OnyxUnitType["position"] | null {
  const digits = String(unitNumber ?? "").trim();
  if (!/^\d{3,}$/.test(digits)) return null;
  const pos = Number(digits.slice(-2));
  return pos >= 1 && pos <= 7 ? (pos as OnyxUnitType["position"]) : null;
}

/** Floor for a flat number: "1204" → 12, "0101" → 1. Null if unreadable. */
export function onyxUnitFloor(unitNumber: string): number | null {
  const digits = String(unitNumber ?? "").trim();
  if (!/^\d{3,}$/.test(digits)) return null;
...
export function onyxUnitType(unitNumber: string): OnyxUnitType | undefined {
  const pos = onyxUnitPosition(unitNumber);
  // A number with no readable floor ("0004") is not a flat in this tower.
  if (!pos || onyxUnitFloor(unitNumber) === null) return undefined;
  return ONYX_UNIT_TYPES.find((t) => t.position === pos);
}
