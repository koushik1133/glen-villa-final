 ...
 */
function publicMediaUrl(pathOrUrl) {
...
async function runTick(now = new Date()) {
    ...
    const result = { checked: 0, published: 0, failed: 0, deferred: 0, details: [] };
    const candidates = db.posts.filter((p) => ["scheduled", "approved", "publishing", "failed"].includes(p.status) && p.scheduledAt);
    for (const post of candidates) {
        ...
        for (const target of targets) {
            const adapter = (0, registry_1.adapterFor)(target.channel);
            ...
            if ((0, connections_1.isUploadPostConnection)(connection)) {
                ...
                const res = await (0, connections_1.publishViaUploadPost)(target.channel, {
            ...
            if (unresolvable) {
                recordFailure(post.id, target.connectionId, `Cannot publish: "${unresolvable}" is a local path and PUBLIC_BASE_URL is not set to a public https address. ${adapter.label} fetches media by URL, so it must be reachable from the internet.`, false);
                result.failed += 1;
...
function updateTarget(postId, connectionId, fn) {
    (0, db_1.mutate)((db) => {
        ...
        if (done && !post.publishedAt)
            post.publishedAt = new Date().toISOString();
        post.updatedAt = new Date().toISOString();
    });
}
function recordSuccess(postId, connectionId, externalId, permalink) {
...
function recordFailure(postId, connectionId, error, retryable) {
    updateTarget(postId, connectionId, (t, p) => {
        t.attempts += 1;
        t.error = error;
        ...
        logActivity(p.brandId, "publish_error", `${t.channel} failed (attempt ${t.attempts}/${MAX_ATTEMPTS}): ${error}${t.status === "failed" ? " — giving up" : " — will retry"}`);
    });
}
function recordDefer(postId, connectionId, reason) {
    updateTarget(postId, connectionId, (t, p) => {
        t.error = reason;
        logActivity(p.brandId, "publish_deferred", `${t.channel} deferred: ${reason}`);
    });
}
function logActivity(brandId, kind, message, actor = "system") {
    (0, db_1.mutate)((db) => {
        db.activity.unshift({ id: (0, ids_1.uid)("act"), brandId, at: new Date().toISOString(), actor, kind, message });
