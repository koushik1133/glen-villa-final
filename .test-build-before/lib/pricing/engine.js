...
exports.applies = applies;
exports.calculate = calculate;
exports.diffVersions = diffVersions;
/**
...
function applies(component, unit) {
    const w = component.appliesWhen;
    if (!w)
        return true;
    if (w.facingIn?.length && (!unit.facing || !w.facingIn.includes(unit.facing)))
        return false;
    if (w.cornerOnly && !unit.corner)
        return false;
    if (w.minFloor !== undefined && unit.floor < w.minFloor)
        return false;
    if (w.minAreaSqft !== undefined && unit.areaSqft < w.minAreaSqft)
        return false;
    if (w.unitTypeIn?.length && (!unit.unitType || !w.unitTypeIn.includes(unit.unitType)))
        return false;
    return true;
}
function describe(component, rate, unit) {
 ...
 */
function calculate(components, unit, overrides = []) {
    ...
    const byCategory = (c) => lines.filter((l) => l.category === c).reduce((s, l) => s + l.amount, 0);
...
function diffVersions(before, after) {
    ...
    const lines = ids.map((id) => {
        ...
        if (!b && a)
            ...
            if (!b.waived && a.waived)
                change = "WAIVED";
            else if (b.waived && !a.waived)
                change = "REINSTATED";
