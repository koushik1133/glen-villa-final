...
function parseAmount(raw) {
    ...
    switch (m[2]) {
        case "cr":
        case "crore":
        case "crores": return n * 1e7;
        case "l":
        case "lac":
        case "lakh":
        case "lakhs": return n * 1e5;
        case "k": return n * 1e3;
        default: return n;
    }
}
/** Token overlap against the row label — tolerant of word order and plurals. */
function score(label, phrase) {
 ...
 */
function parseInstruction(instruction, components, unit, currentOverrides = []) {
    const text = instruction.trim();
    if (!text)
        return { kind: "UNRECOGNISED", message: "Nothing to do." };
    /* ---- Unit attributes ------------------------------------------------- */
    ...
    if (areaMatch && /\b(change|set|make|update|to|is)\b/i.test(text)) {
        ...
        if (value)
            return { kind: "SET_UNIT", field: "areaSqft", value, previous: unit.areaSqft };
    }
    ...
    if (valueMatch) {
        ...
        if (value === null)
            return { kind: "UNRECOGNISED", message: `I could not read "${valueMatch[1]}" as an amount.` };
...
function applyIntent(intent, overrides) {
