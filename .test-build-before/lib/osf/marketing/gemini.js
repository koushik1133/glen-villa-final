...
exports.inputsFromProject = inputsFromProject;
exports.generateContent = generateContent;
const genai_1 = require("@google/genai");
const env_1 = require("../env");
/** Pulls the strings the generator needs out of real DB records. */
function inputsFromProject(project, villaType) {
    ...
    const amenities = Array.isArray(project.usps)
        ? project.usps.filter((u) => typeof u === "string").slice(0, 6)
        : undefined;
...
function fallbackContent(params) {
    const { format, projectName, location = "Hyderabad", price = "price on request", bhk = "3 & 4 BHK", amenities = [], } = params;
...
const TONE_BRIEF = {
    ultra_luxury: "Restrained and aspirational. Short sentences, no exclamation marks, no emoji spam. Sell the feeling of arrival, not the discount.",
    urgency_scarcity: "Direct and time-bound. Reference only scarcity that is stated in the property details above — if no unit count or deadline is given, create urgency from the buying decision itself, never from an invented 'only 3 left'.",
    investor_roi: "Analytical. Talk corridor growth, rental demand and holding horizon. Do not quote an appreciation percentage or rental yield — no such figure has been supplied.",
    nri_special: "Written for a buyer who cannot walk the site. Emphasise virtual walkthroughs, documentation, and the sales team handling things locally. No tax or FEMA claims.",
    architectural_spotlight: "Design-led. Materials, proportion, light, elevation, landscaping. Describe what an architect would notice.",
};
...
async function generateContent(params) {
    const apiKey = env_1.env.geminiApiKey;
    if (!apiKey)
