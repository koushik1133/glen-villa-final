...
exports.enrollLead = enrollLead;
exports.autoEnroll = autoEnroll;
exports.runDueSequenceSteps = runDueSequenceSteps;
...
async function enrollLead(sequenceId, leadId) {
    ...
    const { data: lead } = await supabase
        ...
        .maybeSingle();
    if (!lead)
        return { enrolled: false, reason: "lead not found" };
    if (lead.opted_out)
        return { enrolled: false, reason: "opted out" };
    if (!lead.phone)
        return { enrolled: false, reason: "no phone number (Instagram-only lead)" };
...
async function autoEnroll() {
    ...
    for (const seq of sequences ?? []) {
        ...
        if (Object.keys(entry).length === 0)
            continue; // manual-only sequence
        const leads = await (0, broadcasts_1.resolveAudience)(entry);
...
async function runDueSequenceSteps() {
    ...
    for (const enrollment of (due ?? [])) {
        ...
        const { data: claimed } = await supabase
            ...
            .maybeSingle();
        if (!claimed)
            continue; // another runner got it
        try {
...
async function fireStep(enrollment) {
    const supabase = (0, supabase_1.db)();
    ...
    const exit = async (status, reason) => {
        await supabase
            .from("villa_sequence_enrollments")
            .update({
            status,
    ...
    const exitStages = sequence.exit_on ?? [];
    ...
    const { data: lastInbound } = await supabase
        ...
        .limit(1)
        .maybeSingle();
    const windowOpen = (0, communication_1.serviceWindow)(lastInbound?.created_at ?? null).open;
    if (windowOpen && step.body) {
        await (0, client_1.sendText)(lead.phone, step.body);
        if (step.asset_id) {
            ...
            if (asset?.shareable_by_ai) {
                await (0, client_1.sendMedia)(lead.phone, asset.url, asset.kind);
            }
        }
    }
        ...
        await (0, client_1.sendTemplate)(lead.phone, template.name, template.language, [
            lead.name ?? "there",
        ]);
    }
    else {
    ...
    const { getOrCreateConversation } = await Promise.resolve().then(() => __importStar(require("./conversation")));
