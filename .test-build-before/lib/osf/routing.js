...
exports.rebalanceUnassigned = rebalanceUnassigned;
exports.rosterLanguages = rosterLanguages;
...
async function routingState() {
    ...
    for (const row of load.rows) {
        ...
        if (CLOSED_STAGES.has(row.pipeline_stage))
            continue;
        assignedOpen += 1;
        open.set(owner, (open.get(owner) ?? 0) + 1);
        if (row.lead_temperature === "hot")
            hot.set(owner, (hot.get(owner) ?? 0) + 1);
    }
 ...
 */
async function pickAssignee(filters = {}) {
    const state = await routingState();
    if (state.eligible.length === 0) {
        return {
            ok: false,
            reason: state.standby.length > 0
    ...
    if (filters.memberId) {
        ...
        if (!pinned)
            return { ok: false, reason: "the pinned rep is inactive or not accepting leads" };
...
async function rebalanceUnassigned() {
    ...
    if (state.eligible.length === 0) {
        return {
            ok: false,
            error: "No active team member is accepting leads. Switch on “accepts leads” for at least one rep first.",
        };
    }
    const { data, error } = await (0, supabase_1.db)()
        .from("villa_leads")
    ...
    const buckets = new Map(state.eligible.map((r) => [r.id, []]));
    for (const lead of queue) {
        const target = state.eligible.reduce((best, r) => (load.get(r.id) ?? 0) < (load.get(best.id) ?? 0) ? r : best);
...
function rosterLanguages(state) {
    const seen = new Set();
