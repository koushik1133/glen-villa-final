...
exports.MIN = void 0;
exports.loadPricingReport = loadPricingReport;
...
async function loadPricingReport() {
    ...
    if (unitsRes.error)
        unavailable.push("unit-level detail");
    if (objectionsRes.error)
        unavailable.push("objections");
    if (typesRes.error)
        unavailable.push("lead villa-type interest");
    const inventory = (inventoryRes.data ?? []);
    const units = (unitsRes.data ?? []);
    const objections = (objectionsRes.data ?? []);
    const interests = (typesRes.data ?? []);
    const interestByType = new Map();
    for (const row of interests) {
        interestByType.set(row.villa_type_interest, (interestByType.get(row.villa_type_interest) ?? 0) + 1);
    }
    const priceObjectionsByType = new Map();
    let totalObjections = 0;
    for (const o of objections) {
        totalObjections += 1;
        if (!PRICE_OBJECTION_PATTERN.test(o.category))
            continue;
    ...
    const facings = [...facingMap.entries()]
        .map(([facing, v]) => ({
        facing,
        units: v.units,
        taken: v.taken,
...
function derive(base) {
    ...
    const blockers = [];
    const comparable = base.types.filter((t) => t.totalUnits >= exports.MIN.unitsPerType && t.absorptionPct !== null);
    // --- Absorption spread between configurations -------------------------------
    if (comparable.length < exports.MIN.typesToCompare) {
        blockers.push(`Absorption comparison needs at least ${exports.MIN.typesToCompare} villa types with ${exports.MIN.unitsPerType}+ units each. ` +
        ...
        if (gap >= exports.MIN.absorptionGapPct && ratio >= exports.MIN.absorptionRatio) {
            const multiple = Number.isFinite(ratio) ? `${round(ratio)}x` : "with no comparable movement";
            recommendations.push({
                id: "absorption-spread",
                category: "absorption",
                tone: "gold",
                observation: `${fastest.villaType} has absorbed ${fast}% of its ${fastest.totalUnits} units. ` +
                    `${slowest.villaType} has absorbed ${slow}% of its ${slowest.totalUnits}. ` +
                    `That is ${multiple} the rate, a ${round(gap)} point gap.`,
                reading: "Two configurations in the same project are moving at very different speeds. The cause could be relative price, plot position, or the mix that was released first — the data here cannot tell which.",
        ...
        for (const t of scarce.slice(0, 2)) {
            recommendations.push({
                ...
                reading: "The last few units of a configuration are the ones with the least competition for the buyer's attention and the most for the developer's.",
                action: "Decide deliberately how the remaining units are released and positioned, rather than letting them go at the same terms as the first ones. Any figure is a sales-team call.",
                evidence: [
    ...
    if (totalInterest < exports.MIN.leadInterestTotal) {
        blockers.push(`Demand ranking needs at least ${exports.MIN.leadInterestTotal} leads with a villa type recorded against them. ` +
        ...
        if (mostWanted.leadInterest > 0 && share !== null) {
            const stalled = mostWanted.totalUnits >= exports.MIN.unitsPerType &&
            ...
            recommendations.push({
                ...
                observation: `${mostWanted.leadInterest} of ${totalInterest} leads with a stated preference (${share}%) name ${mostWanted.villaType}` +
    ...
    if (typedPriceObjections < exports.MIN.priceObjections) {
        blockers.push(`Price-objection analysis needs at least ${exports.MIN.priceObjections} price objections from leads with a villa type recorded. ` +
        ...
        recommendations.push({
            ...
            category: "objections",
            tone: "warning",
            observation: `${top.priceObjections} of ${typedPriceObjections} price objections (${share}%) come from leads interested in ${top.villaType}` +
                (top.absorptionPct !== null ? `, which is ${top.absorptionPct}% absorbed.` : "."),
            reading: "Price resistance is not spread evenly across the range. It concentrates on one configuration, which is either a positioning problem or a value-communication problem — the objection text says which, this count does not.",
    ...
    if (base.cornerUnits === 0 && unitsWithExtra === 0) {
        blockers.push("Premium analysis needs units carrying chargeable extra area or flagged as corner plots. None are recorded — set is_corner and chargeable_extra_sqyd in Inventory.");
    }
    else if (base.cornerUnits >= exports.MIN.unitsWithExtra && base.standardUnits >= exports.MIN.unitsWithExtra) {
        ...
        if (cornerPct !== null && standardPct !== null) {
            ...
            recommendations.push({
                ...
                observation: `Corner units are ${cornerPct}% taken (${base.cornerTaken} of ${base.cornerUnits}) against ${standardPct}% for standard units (${base.standardTaken} of ${base.standardUnits})` +
    ...
    // --- Facing spread ---------------------------------------------------------
    const comparableFacings = base.facings.filter((f) => f.units >= exports.MIN.unitsPerFacing && f.takenPct !== null);
    if (comparableFacings.length >= 2) {
        ...
        if (gap >= exports.MIN.absorptionGapPct && ratio >= exports.MIN.absorptionRatio) {
            ...
            recommendations.push({
                ...
                reading: "Facing is priced the same across most plots here unless chargeable extra area says otherwise, but buyers are clearly not treating the orientations as equivalent.",
        ...
        blockers.push(`Facing comparison needs at least two facings with ${exports.MIN.unitsPerFacing}+ units each. Set the facing on each unit in Inventory.`);
    }
    if (base.totalUnits === 0) {
        blockers.length = 0;
        blockers.push("No units are recorded yet. Every recommendation on this page is derived from unit-level inventory — add units in Inventory and they will appear.");
    }
