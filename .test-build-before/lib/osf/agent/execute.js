...
exports.deliverApprovedAssets = deliverApprovedAssets;
exports.executeTool = executeTool;
const supabase_1 = require("../supabase");
const env_1 = require("../env");
const safe_url_1 = require("../net/safe-url");
const deliverable_1 = require("../net/deliverable");
...
const THROTTLED = "That action is not available again right now. Do not retry it and do not mention " +
    "any limit to the customer — carry on with the conversation normally.";
function overBudget(conversationId, tool) {
    if (!takeBudget(conversationId, CONVERSATION_TOOL_BUDGET))
        return true;
    ...
    return [
        {
            ...
            body: `Status: ${p.status ?? "not recorded"}. Expected delivery: ${p.expected_delivery ?? "not recorded"}. Land area: ${p.total_land_acres ?? "not recorded"} acres. Total villas: ${p.total_units ?? "not recorded"}. Configurations: ${p.configurations?.join(", ") ?? "not recorded"}. ${p.positioning ?? ""}`,
        },
        {
            ...base,
        ...
        },
        {
            ...
            body: `HMDA permit: ${p.hmda_permit_no ?? "not recorded"}${p.hmda_permit_date ? ` dated ${p.hmda_permit_date}` : ""}. RERA: ${p.rera_number ?? p.rera_status ?? "not recorded"}.`,
        },
        {
            ...base,
            heading: "Pricing",
        ...
        { ...base, heading: "Sustainability and certification", body: flattenValue(p.sustainability) },
        {
            ...
            body: `${flattenValue(p.connectivity)}. These are the developer's own published estimates — always add that actual travel time varies with traffic.`,
        },
        {
            ...base,
        ...
        },
        {
            ...
            body: `Assistance is offered through: ${p.financing_partners?.join(", ") ?? "not recorded"}. Loan approval is always at the bank's discretion and is never guaranteed.`,
        },
...
async function searchKnowledgeBase(input) {
    const { projects, faqs } = await (0, kb_1.loadKb)();
    ...
    // Top 3, each trimmed. On Groq's free tier the tool result stays in the
    // message array and is resent on every later turn, so 5 long sections was a
    // major, compounding token cost. 3 focused snippets answer the question
    // without pushing the turn past the per-minute budget.
    const matches = candidates.slice(0, 3).map(({ label, content }) => ({
        label,
...
async function getVillaTypes(input) {
    const { projects, villaTypes } = await (0, kb_1.loadKb)();
    const scoped = input.project_slug
        ? projects.filter((p) => p.slug === input.project_slug)
        : projects;
    ...
    return {
        ...
        note: "Where bedrooms or price read 'not recorded', that value is not approved for you to state. Offer to have the sales team confirm it.",
    };
}
async function checkAvailability(input) {
    ...
    if (!data || data.length === 0) {
        return {
            ...
            note: "No live inventory is connected for this project. Tell the customer you can help identify the right villa type and that the sales team will confirm current availability. Do NOT state a count, a unit number, or that anything is 'selling fast'.",
        };
    }
...
async function getAssets(input) {
    ...
    const { data, error } = await query;
    if (error)
        return { ok: false, error: "asset lookup failed" };
    const rows = data ?? [];
    ...
    if (input.kind === "location_map") {
        const out = { ok: true, assets: rows.map(shapeAsset) };
        if (env_1.env.projectMapsUrl) {
            out.map_link = env_1.env.projectMapsUrl;
            out.note =
                `Send this Google Maps link to the customer as plain text in your reply, copied EXACTLY: ${env_1.env.projectMapsUrl} — ` +
    ...
    if (rows.length === 0) {
        ...
        if (input.kind === "brochure" && env_1.env.brochureUrl) {
            if (await (0, deliverable_1.isDeliverableFile)(env_1.env.brochureUrl, false)) {
                return {
                    ok: true,
                    assets: [{ title: "Project brochure", url: env_1.env.brochureUrl, is_ai_generated: false }],
                };
            }
        }
        return { ok: true, assets: [], note: NO_ASSET_NOTE };
    ...
    if (input.kind === "brochure" && rows.length > 1) {
        notes.unshift(`There are ${rows.length} approved brochures. When the customer asks for "the brochure", send ALL of them — ` +
            "one send_media call each, in the order listed.");
    }
    return { ok: true, assets: rows.map(shapeAsset), note: notes.join(" ") };
}
const NO_ASSET_NOTE = "No approved asset of this kind is uploaded yet. Tell the customer you will have the sales team send it across, and call log_unanswered_question. Do not invent a link.";
 ...
 */
async function sendOptions(ctx, input) {
    const body = cleanText(input.body, 1024);
    if (!body)
        return { ok: false, error: "send_options needs a body — the question to ask." };
    const seen = new Set();
    const options = [];
    ...
    if (options.length < 2) {
        return {
            ok: false,
            error: "send_options needs at least 2 distinct choices. Ask the question as plain text instead.",
        };
    }
    try {
        await ctx.deliver({
            text: body,
            options,
            listButtonLabel: options.length > 3 ? (cleanText(input.list_button_label, 20) ?? "Choose") : undefined,
        });
    }
    catch {
        return {
            ok: false,
            error: "Those options could not be delivered. Ask the same question as plain text instead, and do not mention a fault.",
        };
    }
 ...
 */
async function deliverApprovedAssets(params) {
    ...
    if (lead.project_interest)
        query = query.eq("project_id", lead.project_interest);
    ...
    for (const row of data) {
        try {
            const { href, source } = await (0, safe_url_1.assertSendableMediaUrl)(row.url);
            if (skip?.has(href))
                continue;
            await (0, deliverable_1.assertDeliverableFile)(href, source === "app_origin");
            const text = (sent === 0 ? caption : undefined) ?? row.title ?? undefined;
...
async function sendMedia(ctx, input) {
    const kind = cleanEnum(input.kind, ASSET_KINDS);
    if (!kind)
        return { ok: false, error: "that is not a kind of asset this agent can send" };
    ...
    if (kind === "location_map" || (0, deliverable_1.isLinkOnlyUrl)(input.url)) {
        return {
            ok: false,
            error: "The location is a link, not a file — it cannot be attached. Put the Google Maps link from get_assets into your reply as plain text, copied exactly. If they want a map to look at, send the master_plan PDF with send_media instead.",
        };
    }
    ...
    let source;
    try {
        ({ href, source } = await (0, safe_url_1.assertSendableMediaUrl)(input.url));
    }
    catch (e) {
        if (e instanceof safe_url_1.UnsafeUrlError) {
            return {
                ok: false,
    ...
    try {
        ...
        if (!(e instanceof deliverable_1.UndeliverableMediaError))
            throw e;
        ...
        if (rescued > 0) {
            return {
                ok: true,
                delivered: true,
                note: `${e.message} The ${rescued} approved ${kind} file(s) were sent instead. ` +
    ...
    try {
        ...
        return {
            ok: false,
            error: "Delivery failed. Tell the customer you are having trouble sending it right now and that the sales team will follow up. Do not claim it was sent.",
        };
    }
    const supabase = (0, supabase_1.db)();
    await supabase.from("villa_messages").insert({
    ...
    const flags = {};
    if (kind === "brochure")
        flags.brochure_sent = true;
    if (kind === "floor_plan")
        flags.floor_plan_sent = true;
    if (kind === "price_sheet")
        flags.price_sheet_sent = true;
    if (kind === "video" || kind === "virtual_tour")
        flags.video_sent = true;
...
async function scheduleSiteVisit(ctx, input) {
    ...
    const { error } = await supabase.from("villa_site_visits").insert({
        ...
        visit_type: cleanEnum(input.visit_type, ["site", "virtual"]) ?? "site",
        special_requirements: cleanText(input.special_requirements, 500) ?? null,
        status: "requested",
    });
    if (error)
        return { ok: false, error: "could not log the visit request" };
    ctx.signals.requestedSiteVisit = true;
    return {
        ok: true,
        status: "requested",
        note: "Logged as a REQUEST. Tell the customer the sales team will confirm the slot shortly. Do not tell them it is already booked or confirmed.",
    };
}
// -----------------------------------------------------------------------------
// Handoff
// -----------------------------------------------------------------------------
function buildHandoffPayload(lead, project, villaType, input) {
    const budget = lead.budget_min_inr || lead.budget_max_inr
...
const ALREADY_HANDED_OFF = "The sales team already has this lead's briefing for this conversation. " +
    "Tell the customer someone will be in touch shortly. Do not create another handoff.";
async function requestHumanHandoff(ctx, input) {
    const supabase = (0, supabase_1.db)();
    ...
    if (countError)
        return { ok: false, error: "could not create the handoff" };
    if ((count ?? 0) > 0)
        return { ok: false, error: ALREADY_HANDED_OFF };
    const { projects, villaTypes } = await (0, kb_1.loadKb)();
    ...
    let notified = false;
    if (env_1.env.salesTeamWhatsapp) {
        try {
            const { sendText } = await Promise.resolve().then(() => __importStar(require("../whatsapp/client")));
            await (0, outbound_1.sendPlainText)(env_1.env.salesTeamWhatsapp, payload);
            notified = true;
            await supabase
                .from("villa_handoffs")
...
// -----------------------------------------------------------------------------
async function logObjection(ctx, input) {
    await (0, supabase_1.db)().from("villa_objections").insert({
        lead_id: ctx.lead.id,
        conversation_id: ctx.conversationId,
...
async function logUnansweredQuestion(ctx, input) {
    await (0, supabase_1.db)().from("villa_questions").insert({
        lead_id: ctx.lead.id,
        conversation_id: ctx.conversationId,
...
const NOT_A_REAL_OPT_OUT = "Nothing in what the customer actually said reads as an opt-out request, so they " +
 ...
 */
async function latestCustomerMessage(conversationId) {
...
async function optOut(ctx, input) {
    ...
    return {
        ok: true,
        opted_out: true,
        note: "Send one short, polite acknowledgement confirming they will not be contacted again, then stop. " +
            (input.verbatim ? "" : ""),
    };
}
// -----------------------------------------------------------------------------
        ...
        try {
            switch (name) {
                case "search_knowledge_base":
                    result = await searchKnowledgeBase(input);
                    break;
                case "get_villa_types":
                    result = await getVillaTypes(input);
                    break;
                case "check_availability":
                    result = await checkAvailability(input);
                    break;
                case "get_assets":
                    result = await getAssets(input);
                    break;
                case "send_options":
                    result = await sendOptions(ctx, input);
                    break;
                case "send_media":
                    result = await sendMedia(ctx, input);
                    break;
                case "update_lead":
                    result = await updateLead(ctx, input);
                    break;
                case "schedule_site_visit":
                    result = await scheduleSiteVisit(ctx, input);
                    break;
                case "request_human_handoff":
                    result = await requestHumanHandoff(ctx, input);
                    break;
                case "log_objection":
                    result = await logObjection(ctx, input);
                    break;
                case "log_unanswered_question":
                    result = await logUnansweredQuestion(ctx, input);
                    break;
                case "opt_out":
                    result = await optOut(ctx, input);
                    break;
                default:
            ...
            result = {
                ok: false,
                error: "This lookup is temporarily unavailable. Tell the customer you will have the sales team confirm, and do not mention a technical fault.",
            };
            console.error(`[tool:${name}]`, e);
        }
    }
    // Audit trail — section 44 requires we never claim a failed action succeeded.
    void (0, supabase_1.db)()
