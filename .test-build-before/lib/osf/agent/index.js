...
async function runAgent(params) {
    if (env_1.env.llmProvider === "groq") {
        const { runAgentGroq } = await Promise.resolve().then(() => __importStar(require("./run-groq")));
        return runAgentGroq(params);
    }
    const { runAgentAnthropic } = await Promise.resolve().then(() => __importStar(require("./run")));
