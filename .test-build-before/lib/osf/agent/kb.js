...
exports.loadKb = loadKb;
exports.knowledgeBaseBlock = knowledgeBaseBlock;
exports.invalidateKbCache = invalidateKbCache;
exports.compactKnowledgeBase = compactKnowledgeBase;
...
async function loadKb() {
    ...
    if (projects.error)
        throw new Error(`Knowledge base unavailable: ${projects.error.message}`);
    return {
...
function money(inr) {
    if (inr === null || inr === undefined)
        return "NOT RECORDED — sales team must confirm";
...
function renderProject(p, types, faqs) {
  ...
  Key selling points:
${Array.isArray(p.usps) ? p.usps.map((u) => `    - ${u}`).join("\n") : "    (none)"}
 ...
 */
async function knowledgeBaseBlock() {
 ...
 */
async function compactKnowledgeBase() {
    ...
    for (const p of projects) {
        ...
        if (p.status)
            lines.push(`Status: ${p.status}${p.expected_delivery ? `, delivery ${p.expected_delivery}` : ""}`);
        if (p.rera_number)
            lines.push(`RERA: ${p.rera_number}`);
        if (p.hmda_permit_no)
            lines.push(`HMDA permit: ${p.hmda_permit_no}`);
        if (p.price_per_sft_inr)
            lines.push(`Base rate: ₹${p.price_per_sft_inr.toLocaleString("en-IN")}/sft`);
        if (p.price_note)
            lines.push(`Price note: ${p.price_note}`);
        if (p.total_land_acres)
            lines.push(`Land: ${p.total_land_acres} acres, ${p.total_units ?? "?"} units`);
        ...
        if (Array.isArray(conn) && conn.length) {
            ...
            const near = conn
                ...
                .join(", ");
            if (near)
                lines.push(`Nearby, with approx drive times: ${near}`);
        }
        ...
        if (si && typeof si === "object") {
            const listOf = (k) => Array.isArray(si[k]) ? si[k].map(String).join(", ") : "";
            const schools = listOf("schools");
            const hospitals = listOf("hospitals");
            const growth = listOf("growth_drivers");
            if (schools)
                lines.push(`Schools nearby: ${schools}`);
            if (hospitals)
                lines.push(`Hospitals nearby: ${hospitals}`);
            if (growth)
                lines.push(`Area growth drivers: ${growth}`);
        }
    }
