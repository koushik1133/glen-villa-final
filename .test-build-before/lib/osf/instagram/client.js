...
exports.sendInstagramText = sendInstagramText;
exports.sendInstagramMedia = sendInstagramMedia;
exports.sendInstagramQuickReplies = sendInstagramQuickReplies;
exports.deliverToInstagram = deliverToInstagram;
...
function endpoint() {
    const version = env_1.env.whatsappApiVersion;
    const account = env_1.env.instagramAccountId;
...
async function post(payload) {
    const response = await fetch(endpoint(), {
        method: "POST",
        headers: {
            Authorization: `Bearer ${env_1.env.instagramAccessToken}`,
...
async function sendInstagramText(to, body) {
    return post({ recipient: { id: to }, message: { text: clamp(body, TEXT_LIMIT) } });
}
async function sendInstagramMedia(to, url, kind) {
...
async function sendInstagramQuickReplies(to, body, options) {
    return post({
        recipient: { id: to },
        message: {
...
async function deliverToInstagram(to, reply) {
    ...
    if (reply.mediaUrl && reply.mediaKind) {
        ...
        if (isDocument) {
            ...
            await sendInstagramText(to, `${reply.caption ? `${reply.caption}\n` : ""}${reply.mediaUrl}`);
            return;
        }
        await sendInstagramMedia(to, reply.mediaUrl, reply.mediaKind);
        if (reply.caption)
            await sendInstagramText(to, reply.caption);
        return;
    }
    if (reply.text)
        await sendInstagramText(to, reply.text);
}
