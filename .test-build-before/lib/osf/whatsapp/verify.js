...
secret) {
    ...
    let expected;
    try {
        expected = node_crypto_1.default
            .createHmac("sha256", secret ?? env_1.env.whatsappAppSecret)
            .update(rawBody, "utf8")
            .digest("hex");
    }
    catch {
...
function timingSafeStringEqual(a, b) {
    ...
    const ha = node_crypto_1.default.createHash("sha256").update(a, "utf8").digest();
    const hb = node_crypto_1.default.createHash("sha256").update(b, "utf8").digest();
    return node_crypto_1.default.timingSafeEqual(ha, hb);
}
/**
...
expectedToken) {
    ...
    if (mode !== "subscribe" || !challenge || token === null)
        return null;
    try {
        // Verify token not configured — fail closed, same as the signature path.
        if (!timingSafeStringEqual(token, expectedToken ?? env_1.env.whatsappVerifyToken))
            return null;
    }
    catch {
        return null;
