...
function endpoint() {
    ...
    const phoneNumberId = env_1.env.whatsappPhoneNumberId;
...
async function sendText(to, body) {
    return post({
        ...
        text: { body: truncate((0, format_1.toWhatsApp)(body), 4096), preview_url: false },
    });
}
...
async function sendMedia(to, url, kind, caption) {
    ...
    const media = { link: link.href };
    if (caption)
        media.caption = truncate(caption, 1024);
    if (type === "document") {
 ...
 */
async function sendTemplate(to, templateName, languageCode = "en", bodyParams = []) {
    return post({
        messaging_product: "whatsapp",
        recipient_type: "individual",
        to: recipient(to),
...
async function sendButtons(to, body, buttons, opts = {}) {
    if (buttons.length === 0)
        throw new Error("sendButtons needs at least one button");
    if (buttons.length > 3) {
 ...
 */
async function sendList(to, body, buttonLabel, sections, opts = {}) {
    const rowCount = sections.reduce((n, s) => n + s.rows.length, 0);
    if (rowCount === 0)
        throw new Error("sendList needs at least one row");
    if (rowCount > 10) {
