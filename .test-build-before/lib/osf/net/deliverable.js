...
exports.UndeliverableMediaError = void 0;
exports.isLinkOnlyUrl = isLinkOnlyUrl;
exports.isPageContentType = isPageContentType;
exports.assertDeliverableFile = assertDeliverableFile;
exports.isDeliverableFile = isDeliverableFile;
 ...
 */
class UndeliverableMediaError extends Error {
    constructor(message) {
        super(message);
        this.name = "UndeliverableMediaError";
    }
}
exports.UndeliverableMediaError = UndeliverableMediaError;
...
function isPageContentType(contentType) {
    if (!contentType)
        return true; // unknown is not "known good"
 ...
 */
function publicFileVerdict(pathname) {
    let decoded;
    try {
    ...
    const root = node_path_1.default.join(process.cwd(), "public");
    const resolved = node_path_1.default.resolve(root, `.${decoded.startsWith("/") ? decoded : `/${decoded}`}`);
    ...
    try {
        ...
        return node_fs_1.default.existsSync(root) ? "missing" : "unknown";
    }
    if (!stat.isFile() || stat.size === 0)
        return "missing";
    return FILE_EXTENSIONS.has(node_path_1.default.extname(resolved).toLowerCase()) ? "file" : "missing";
}
// -----------------------------------------------------------------------------
// Everything else: probe it
// -----------------------------------------------------------------------------
async function probe(href, method) {
 ...
 */
async function assertDeliverableFile(href, isAppOrigin) {
    if (isLinkOnlyUrl(href)) {
        throw new UndeliverableMediaError("that link is a map/web page, not a file — it cannot be sent as a document. Send it to the customer as a plain text link in your reply instead.");
    }
    if (isAppOrigin) {
        ...
        if (verdict === "file")
            return;
        if (verdict === "missing") {
            throw new UndeliverableMediaError("no file exists at that path — you invented or mistyped the URL.");
        }
    ...
    if (isPageContentType(response.headers.get("content-type"))) {
        throw new UndeliverableMediaError("that link serves a web page, not a file — sending it would deliver an unreadable document.");
    }
}
/** Convenience wrapper: true when the URL really serves a file. */
async function isDeliverableFile(href, isAppOrigin) {
    try {
