...
const FORMATS = ["reel", "carousel", "feed", "story", "short"];
/** Pretty project name from the inventory's slug. */
function projectName(slug) {
    if (slug === "serenity")
        return "Glentree Serenity";
    if (slug === "onyx")
        return "Glentree Onyx";
...
function daysUntil(month, day, now) {
    ...
    if (target.getTime() < now.getTime() - 3 * 86400000)
        target = new Date(year + 1, month - 1, day);
    return Math.round((target.getTime() - now.getTime()) / 86400000);
}
function idea(brandId, parts) {
    return {
        id: (0, ids_1.uid)("idea"),
        brandId,
 ...
 */
function generateIdeas(db, brandId, now = new Date()) {
    const out = [];
    const read = [];
    ...
    for (const [slug, list] of byProject) {
        ...
        const sizes = available.map((u) => u.plotSqYds).filter((n) => typeof n === "number");
        if (sizes.length >= 3) {
