...
exports.DEFAULT_EDIT = exports.ASPECTS = void 0;
exports.aspectForFormat = aspectForFormat;
exports.hasFfmpeg = hasFfmpeg;
exports.buildFilterGraph = buildFilterGraph;
exports.buildFfmpegArgs = buildFfmpegArgs;
exports.renderAspect = renderAspect;
exports.renderForFormats = renderForFormats;
...
function aspectForFormat(format) {
    if (format === "reel" || format === "story" || format === "short")
        return "9:16";
    if (format === "feed")
        return "4:5";
    return "16:9";
 ...
 */
function buildFilterGraph(edit, aspect, source) {
    const { w, h } = exports.ASPECTS[aspect];
    const filters = [];
    if (edit.speed !== 1)
        filters.push(`setpts=${(1 / edit.speed).toFixed(4)}*PTS`);
    ...
    for (const o of edit.overlays) {
        ...
        filters.push(`drawtext=text='${text}':fontsize=${Math.round(o.size * (w / 1080))}:fontcolor=${o.color}:` +
    ...
    if (edit.captionsEnabled) {
        ...
        const style = edit.captionStyle === "bold-center"
...
function buildFfmpegArgs(input, output, edit, aspect, source, srtPath) {
 ...
 */
async function renderAspect(asset, edit, aspect, outDir = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME
    ? node_path_1.default.join("/tmp", "renders")
    : node_path_1.default.join(process.cwd(), "public", "renders")) {
    const key = hashRecipe(asset.id, edit, aspect);
    const outputPath = node_path_1.default.join(outDir, `${key}.mp4`);
    ...
    const readable = remote || node_fs_1.default.existsSync(asset.src);
    ...
    const ok = await new Promise((resolve) => {
        const proc = (0, node_child_process_1.spawn)("ffmpeg", args, { stdio: "ignore" });
