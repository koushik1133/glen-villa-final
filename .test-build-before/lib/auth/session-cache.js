...
exports.MAX_ENTRIES = exports.FAILURE_TTL_MS = exports.TTL_MS = void 0;
exports.cacheKeyFor = cacheKeyFor;
exports.cacheKeyFromCookies = cacheKeyFromCookies;
exports.extractAccessToken = extractAccessToken;
exports.hasRefreshToken = hasRefreshToken;
exports.isObviouslyExpired = isObviouslyExpired;
...
exports.sessionCacheSize = sessionCacheSize;
 ...
 */
exports.TTL_MS = 30_000;
exports.FAILURE_TTL_MS = 5_000;
exports.MAX_ENTRIES = 500;
...
function cacheKeyFor(cookieValue) {
    return (0, node_crypto_1.createHash)("sha256").update(cookieValue).digest("hex");
}
/**
 ...
 */
function cacheKeyFromCookies(all) {
    const parts = all
 ...
 */
function hasRefreshToken(cookieValue) {
    let raw = cookieValue;
    if (raw.startsWith("base64-")) {
        try {
            raw = Buffer.from(raw.slice(7), "base64").toString("utf8");
    ...
    try {
        ...
        if (Array.isArray(parsed))
            return typeof parsed[1] === "string" && parsed[1].length > 0;
 ...
 */
function isObviouslyExpired(accessToken, now = Date.now()) {
...
function getCachedSession(key, now = Date.now()) {
...
function setCachedSession(key, result, now = Date.now()) {
    const ttl = result.status === "active" ? exports.TTL_MS : exports.FAILURE_TTL_MS;
    store.delete(key);
    store.set(key, { result, expiresAt: now + ttl });
    while (store.size > exports.MAX_ENTRIES) {
