...
exports.configuredAgentId = configuredAgentId;
exports.normaliseAgent = normaliseAgent;
exports.turnsFromText = turnsFromText;
exports.normaliseExecution = normaliseExecution;
...
exports.checkBolnaStatus = checkBolnaStatus;
...
function providerMessage(body) {
    ...
    try {
        ...
        if (r) {
            ...
            if (direct)
                return redact(direct.slice(0, 300));
            ...
            if (msg)
                return redact(msg.slice(0, 300));
        }
        if (typeof parsed === "string" && parsed.trim())
            return redact(parsed.slice(0, 300));
    }
    catch {
...
function languagesFrom(sources) {
    const out = [];
    const add = (v) => {
        const s = str(v);
        if (s && !out.includes(s))
    ...
    for (const source of sources) {
        if (!source)
            continue;
        add(source.language);
        for (const item of list(source.languages))
            add(item);
    }
    return out;
}
function normaliseAgent(input) {
    ...
    return {
        ...
        voice: voiceProvider || voiceName || voiceModel
...
function turnsFromText(input) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (m && m[2].trim())
            turns.push({ role: m[1].trim().toLowerCase(), text: m[2].trim() });
        else if (line.trim() && turns.length)
            turns[turns.length - 1].text += ` ${line.trim()}`;
    }
    return turns.length ? turns : null;
}
function turnsFrom(input) {
...
function normaliseExecution(input) {
    const r = rec(input);
    if (!r)
        return null;
    ...
    return {
        ...
        durationSeconds: pickNum([r], "conversation_duration", "conversation_time", "duration") ?? pickNum([telephony], "duration"),
...
async function getAgent(agentId) {
    ...
    if (!agent)
        return { ok: false, reason: "shape", error: "Bolna returned no readable agent for that id." };
...
async function updateAgent(agentId, patch) {
    ...
    if (patch.name !== undefined)
        agentConfig.agent_name = patch.name;
    if (patch.welcomeMessage !== undefined)
        agentConfig.agent_welcome_message = patch.welcomeMessage;
    if (patch.webhookUrl !== undefined)
        agentConfig.webhook_url = patch.webhookUrl;
    const body = {};
    if (Object.keys(agentConfig).length)
        body.agent_config = agentConfig;
    if (patch.systemPrompt !== undefined)
        body.agent_prompts = { task_1: { system_prompt: patch.systemPrompt } };
    if (!Object.keys(body).length)
        return { ok: true, data: { state: "unchanged" } };
...
async function getExecution(executionId) {
    ...
    if (!execution)
        return { ok: false, reason: "shape", error: "Bolna returned no readable execution for that id." };
    return { ok: true, data: execution };
}
/** GET /agent/{agent_id}/executions — call history for one agent. */
async function listExecutions(agentId) {
...
async function getAccount() {
    ...
    const empty = account.balance === null && account.currency === null && account.plan === null && account.email === null;
...
async function checkBolnaStatus() {
    if (!isConfigured()) {
        return {
            configured: false,
            valid: false,
