...
exports.quoteOf = quoteOf;
exports.isLegacyDataUrl = isLegacyDataUrl;
...
function parseMessage(body) {
    ...
    if (target.startsWith(IMAGE)) {
        const p = safeParse(target.slice(IMAGE.length));
        if (p?.media)
            return { type: "image", ...p, kind: "image" };
        return { type: "text", text: target };
    }
    if (target.startsWith(VOICE)) {
        const p = safeParse(target.slice(VOICE.length));
        if (p?.media)
            return { type: "voice", ...p, kind: "voice" };
        return { type: "text", text: target };
    }
    if (target.startsWith(STRUCTURED)) {
        const p = safeParse(target.slice(STRUCTURED.length));
        if (!p)
            return { type: "text", text: target };
...
function formatText(text, replyTo, reactions) {
    const hasExtras = Boolean(replyTo) || Boolean(reactions && Object.keys(reactions).length);
    if (!hasExtras)
        return text.trim();
    const payload = { text: text.trim(), replyTo, reactions };
...
function summarise(body, senderName) {
    const parsed = parseMessage(body);
    if (parsed.type === "image")
        return parsed.caption ? `📷 ${parsed.caption}` : `📷 Photo${senderName ? ` from ${senderName}` : ""}`;
...
function quoteOf(message, senderName) {
...
function isLegacyDataUrl(media) {
