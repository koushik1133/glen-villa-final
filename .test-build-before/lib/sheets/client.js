...
exports.googleSheetsApiKey = googleSheetsApiKey;
exports.isGoogleSheetsConfigured = isGoogleSheetsConfigured;
exports.extractSpreadsheetId = extractSpreadsheetId;
exports.checkGoogleSheetsStatus = checkGoogleSheetsStatus;
exports.getSpreadsheetMetadata = getSpreadsheetMetadata;
exports.readSheetValues = readSheetValues;
exports.parseLeadsFromSheet = parseLeadsFromSheet;
 ...
 */
async function getSpreadsheetMetadata(spreadsheetIdOrUrl) {
    const key = googleSheetsApiKey();
    if (!key)
 ...
 */
async function readSheetValues(spreadsheetIdOrUrl, range = "A1:Z100") {
    const key = googleSheetsApiKey();
    if (!key)
        throw new Error("GOOGLE_SHEETS_API_KEY is not configured");
    const id = extractSpreadsheetId(spreadsheetIdOrUrl);
 ...
 */
function parseLeadsFromSheet(rows, brandId = "brand_glentree") {
    ...
    for (let r = 1; r < rows.length; r++) {
        ...
        let status = "new";
        if (rawStage.includes("visit"))
            status = "site_visit_scheduled";
        else if (rawStage.includes("contact"))
            status = "contacted";
        else if (rawStage.includes("negotiat"))
            status = "negotiation";
        else if (rawStage.includes("token"))
            status = "booking_token_paid";
        else if (rawStage.includes("won") || rawStage.includes("close"))
            status = "won";
        ...
        const source = validSources.find((s) => s === rawSource) || "website";
