...
async function draftReply(brand, review) {
...
function fallbackReply(brand, review) {
    ...
    return (`${name}, I'm sorry — this isn't the standard we hold ourselves to. ` +
...
function analyseReview(text, rating) {
    const lower = text.toLowerCase();
    const neg = NEGATIVE.filter((w) => lower.includes(w)).length;
    const pos = POSITIVE.filter((w) => lower.includes(w)).length;
    // Rating dominates; the lexicon only breaks ties on 3★.
    const sentiment = rating >= 4 ? "positive" : rating <= 2 ? "negative" : pos > neg ? "positive" : neg > pos ? "negative" : "neutral";
