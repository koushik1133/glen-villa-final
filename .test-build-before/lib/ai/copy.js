...
async function generateCopy(req) {
    const llm = (0, provider_1.hasLLM)() ? await llmCopy(req) : null;
    if (llm?.length)
        return llm.map((v) => withLimitCheck(v));
    return req.channels.map((channel) => withLimitCheck(templateCopy(req, channel)));
}
function withLimitCheck(v) {
    const caps = (0, registry_1.adapterFor)(v.channel)?.capabilities;
    const len = [v.caption, v.hashtags.map((h) => `#${h}`).join(" ")].join(" ").length;
    return { ...v, withinLimit: !caps || len <= caps.captionLimit };
}
async function llmCopy(req) {
    const constraints = req.channels
        .map((c) => {
        const caps = (0, registry_1.adapterFor)(c)?.capabilities;
    ...
    const text = await (0, provider_1.complete)({
        system: "You are a senior social copywriter. You write copy that sounds like a person, not a brand deck. " +
...
Return a JSON array. Each item: {"channel": string, "hook": "the first line, must earn the second", "caption": string, "hashtags": string[] (no # prefix), "firstComment": string or null}`,
    ...
    return (0, provider_1.extractJson)(text);
}
/**
 ...
 */
function templateCopy(req, channel) {
    const caps = (0, registry_1.adapterFor)(channel)?.capabilities;
...
async function generateHooks(brand, topic, weakHook) {
    const text = await (0, provider_1.complete)({
...
Write 5 replacement hooks. Vary the mechanism: curiosity gap, direct benefit, contradiction, question, number.`,
    ...
    const parsed = (0, provider_1.extractJson)(text);
    if (parsed?.length)
        return parsed;
