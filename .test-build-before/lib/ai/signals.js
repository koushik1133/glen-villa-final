...
exports.previousRange = void 0;
exports.creativeFatigue = creativeFatigue;
exports.budgetReallocation = budgetReallocation;
exports.boostOrganic = boostOrganic;
exports.postingTime = postingTime;
exports.formatMix = formatMix;
exports.hookQuality = hookQuality;
exports.anomalies = anomalies;
exports.pacing = pacing;
exports.reviewResponse = reviewResponse;
exports.localVisibility = localVisibility;
exports.competitorGap = competitorGap;
exports.queueHealth = queueHealth;
exports.generateSuggestions = generateSuggestions;
const aggregate_1 = require("../metrics/aggregate");
Object.defineProperty(exports, "previousRange", { enumerable: true, get: function () { return aggregate_1.previousRange; } });
...
function make(brandId, s) {
    return { ...s, id: (0, ids_1.uid)("sug"), brandId, createdAt: new Date().toISOString(), state: "new" };
}
function money(n) {
 ...
 */
function creativeFatigue(db, brandId, range) {
    const rows = db.adStats.filter((r) => r.brandId === brandId && (0, aggregate_1.inRange)(r.date, range));
    const byAd = new Map();
    for (const r of rows) {
    ...
    for (const [adId, adRows] of byAd) {
        ...
        if (spend < 100)
            continue; // not enough money at stake to bother a human
        const dailyCtr = sorted.map((r) => (r.impressions ? (r.clicks / r.impressions) * 100 : 0));
        const freq = (0, aggregate_1.adTotals)(sorted).frequency;
        const ctrSlope = (0, stats_1.trendSlopePct)(dailyCtr);
        const cpmEarly = (0, aggregate_1.adTotals)(sorted.slice(0, Math.floor(sorted.length / 2)));
        const cpmLate = (0, aggregate_1.adTotals)(sorted.slice(Math.floor(sorted.length / 2)));
        const cpmDrift = cpmEarly.cpm ? ((cpmLate.cpm - cpmEarly.cpm) / cpmEarly.cpm) * 100 : 0;
        if (freq < 2.8 || ctrSlope > -1.5)
            continue;
        ...
        out.push(make(brandId, {
            kind: "creative_fatigue",
            severity: freq > 4 || cpmDrift > 25 ? "critical" : "opportunity",
            title: `"${ad.name}" is burning out`,
            rationale: `Frequency has reached ${freq.toFixed(1)}x and daily CTR is falling ${Math.abs(ctrSlope).toFixed(1)}% per day ` +
 ...
 */
function budgetReallocation(db, brandId, range) {
    const rows = db.adStats.filter((r) => r.brandId === brandId && (0, aggregate_1.inRange)(r.date, range));
    ...
    for (const campaign of campaigns) {
        const perSet = campaign.adSets
            ...
            const t = (0, aggregate_1.adTotals)(setRows);
            return {
                set,
                t,
                score: (0, stats_1.wilsonLower)(t.conversions, Math.max(t.clicks, t.conversions)) * (t.roas || 0.01),
            };
        })
        ...
        if (best.t.roas < worst.t.roas * 1.4)
        ...
        out.push(make(brandId, {
            ...
            rationale: `Inside ${campaign.name}, "${best.set.name}" returns ${best.t.roas.toFixed(2)}x on ${money(best.t.spend)} ` +
 ...
 */
function boostOrganic(db, brandId) {
    ...
    const baseline = (0, stats_1.mean)(history);
    const recent = published.filter((p) => Date.now() - new Date(p.publishedAt).getTime() < 10 * DAY_MS);
    const out = [];
    for (const p of recent) {
        const z = (0, stats_1.robustZ)(p.metrics.engagementRate, history);
        if (z < 1.5)
            continue;
        ...
        const blendedCpm = (0, aggregate_1.adTotals)(db.adStats.filter((r) => r.brandId === brandId)).cpm;
        if (!blendedCpm)
            continue;
        ...
        out.push(make(brandId, {
            ...
            rationale: `This post is running at ${p.metrics.engagementRate.toFixed(1)}% engagement — ${lift.toFixed(1)}x your ` +
...
function postingTime(db, brandId) {
    ...
    const overall = (0, stats_1.mean)(posts.map((p) => p.metrics.engagementRate));
    const ranked = [...cells.entries()]
        .filter(([, c]) => c.count >= 2)
        .map(([key, c]) => ({ key, avg: (0, stats_1.mean)(c.er), count: c.count }))
        .sort((a, b) => b.avg - a.avg);
    if (!ranked.length)
        return [];
    const best = ranked[0];
    if (best.avg < overall * 1.25)
        return [];
    ...
    return [
        make(brandId, {
            ...
            rationale: `Across ${posts.length} published posts, the ${dayName} ${String(hour).padStart(2, "0")}:00 slot averages ` +
...
function formatMix(db, brandId) {
    ...
    const ranked = [...byFormat.entries()]
        .filter(([, v]) => v.n >= 3)
        .map(([format, v]) => ({ format, er: (0, stats_1.mean)(v.er), reach: (0, stats_1.mean)(v.reach), n: v.n }))
    ...
    if (best.reach < worst.reach * 1.5 || share > 55)
        return [];
    ...
    return [
        make(brandId, {
            ...
            rationale: `Your ${best.format} posts average ${Math.round(best.reach).toLocaleString()} reach and ${best.er.toFixed(1)}% engagement ` +
 ...
 */
function hookQuality(db, brandId) {
    const videos = db.posts.filter((p) => p.brandId === brandId &&
        p.status === "published" &&
        p.metrics &&
    ...
    const avg = (0, stats_1.mean)(retentions);
    const weak = videos.filter((p) => p.metrics.retention3s < 0.45);
    if (weak.length < Math.max(2, videos.length * 0.3))
        return [];
    const strong = videos.filter((p) => p.metrics.retention3s >= 0.65);
    const strongReach = (0, stats_1.mean)(strong.map((p) => p.metrics.reach)) || 0;
    const weakReach = (0, stats_1.mean)(weak.map((p) => p.metrics.reach)) || 1;
    return [
        make(brandId, {
            kind: "hook_quality",
            severity: "critical",
            title: `${weak.length} of ${videos.length} reels lose the viewer in 3 seconds`,
            rationale: `Average 3-second retention is ${(avg * 100).toFixed(0)}%. The ${weak.length} weakest reels sit below 45%, and they reach ` +
...
function anomalies(db, brandId) {
    ...
    for (const [label, series, unit] of [
        ...
        const z = (0, stats_1.robustZ)(last, history);
        if (Math.abs(z) < 2.5)
            continue;
        const expected = (0, stats_1.mean)(history);
        out.push(make(brandId, {
            kind: "anomaly",
            ...
            rationale: `${label} came in at ${Math.round(last).toLocaleString()} against a 30-day median of ${Math.round(expected).toLocaleString()} ` +
...
function pacing(db, brandId) {
    ...
    for (const c of db.adCampaigns.filter((c) => c.brandId === brandId && c.status === "active")) {
        ...
        out.push(make(brandId, {
            ...
            rationale: `Planned budget is ${money(planned)} over ${Math.round(totalDays)} days. After ${Math.round(elapsedDays)} days you have spent ` +
 ...
 */
function reviewResponse(db, brandId) {
    ...
    if (negatives.length) {
        ...
        out.push(make(brandId, {
            ...
            rationale: `The oldest has been sitting ${ageDays} days. Negative reviews are read disproportionately by people close to booking, ` +
    ...
    if (rate < 70 && unreplied.length > 3) {
        out.push(make(brandId, {
            ...
            rationale: `You have answered ${reviews.length - unreplied.length} of ${reviews.length} reviews. Businesses that answer consistently ` +
...
function localVisibility(db, brandId) {
    ...
    for (const c of cells)
        byKeyword.set(c.keyword, [...(byKeyword.get(c.keyword) ?? []), c]);
    const out = [];
    for (const [keyword, kwCells] of byKeyword) {
        const ranks = kwCells.map((c) => c.rank);
        const avg = (0, stats_1.mean)(ranks);
        ...
        out.push(make(brandId, {
            ...
            rationale: `Across a ${rows}x${cols} grid your average position is ` +
...
function edgeFalloff(cells) {
    ...
    const isEdge = (c) => c.row === minRow || c.row === maxRow || c.col === minCol || c.col === maxCol;
    ...
    const edgeAvg = (0, stats_1.mean)(edge.map((c) => c.rank));
    const innerAvg = (0, stats_1.mean)(inner.map((c) => c.rank));
    ...
    if (gap >= 1) {
        return (`The outer ring averages position ${edgeAvg.toFixed(1)} against ${innerAvg.toFixed(1)} nearer the pin, ` +
    ...
    return (`Edge cells average ${edgeAvg.toFixed(1)} and interior cells ${innerAvg.toFixed(1)}, ` +
        `so the weakness is spread across the grid rather than concentrated at its edges.`);
}
/* ------------------------------------------------------------------------- */
/* 11. Competitor gap                                                         */
/* ------------------------------------------------------------------------- */
function competitorGap(db, brandId, range) {
    const comps = db.competitors.filter((c) => c.brandId === brandId);
    if (!comps.length)
        return [];
    const stats = db.dailyStats.filter((s) => s.brandId === brandId && (0, aggregate_1.inRange)(s.date, range));
    const myPosts = stats.reduce((a, s) => a + s.posts, 0);
    const days = daysIn(range);
    const myPerWeek = (myPosts / days) * 7;
    const theirPerWeek = (0, stats_1.mean)(comps.map((c) => c.postsPerWeek));
    ...
    if (theirPerWeek > myPerWeek * 1.5) {
        out.push(make(brandId, {
            ...
            rationale: `You are averaging ${myPerWeek.toFixed(1)} posts/week across all channels; the ${comps.length} tracked competitors average ` +
...
function queueHealth(db, brandId) {
    const scheduled = db.posts.filter((p) => p.brandId === brandId && (p.status === "scheduled" || p.status === "approved") && p.scheduledAt);
    const horizonDays = scheduled.length
        ? Math.max(0, (new Date([...scheduled].sort((a, b) => b.scheduledAt.localeCompare(a.scheduledAt))[0].scheduledAt).getTime() -
            Date.now()) /
            DAY_MS)
        : 0;
    ...
    if (failed.length) {
        out.push(make(brandId, {
            ...
            rationale: `These were scheduled but the platform rejected or timed out on them: ` +
    ...
    out.push(make(brandId, {
        ...
        title: scheduled.length === 0
            ? "Your queue is empty"
            : `Only ${horizonDays.toFixed(1)} days of content scheduled`,
        rationale: `${scheduled.length} post${scheduled.length === 1 ? "" : "s"} remain in the queue. Reach decay after a publishing gap is ` +
 ...
 */
function generateSuggestions(db, brandId, range = (0, aggregate_1.lastNDays)(30)) {
    const all = [
    ...
    return all
        .map((s) => ({
        s,
        score: severityRank(s.severity) * 1000 +
