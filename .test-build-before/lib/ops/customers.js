...
exports.automationAllowed = automationAllowed;
...
function findByPhone(orgId, phone) {
    const key = normalisePhone(phone);
    return (0, db_1.read)().customers.find((c) => c.orgId === orgId && normalisePhone(c.phone) === key);
}
/**
...
function upsertCustomer(input) {
    ...
    (0, db_1.mutate)((db) => void db.customers.push(customer));
    (0, audit_1.audit)({
        orgId: input.orgId,
        actorType: "system",
...
function getCustomer(customerId) {
    return (0, db_1.read)().customers.find((c) => c.id === customerId);
}
/** Patch the profile and record what changed, by whom. */
function updateCustomer(customerId, patch, actor) {
    ...
    if (!after)
        return null;
    const changed = Object.keys(safe).filter((k) => JSON.stringify(asRecord(before)[k]) !== JSON.stringify(asRecord(after)[k]));
    if (changed.length) {
        (0, audit_1.audit)({
            orgId: before.orgId,
            actorId: actor.id,
...
function setStage(customerId, stage, actor, reason) {
    const before = getCustomer(customerId);
    if (!before || before.leadStage === stage)
        return before ?? null;
 ...
 * and vice versa.
 */
function setControl(customerId, lane, state, actor) {
    const field = lane === "SALES" ? "salesControl" : "loanControl";
    const before = getCustomer(customerId);
    if (!before)
        return null;
    const after = updateCustomer(customerId, { [field]: state }, actor);
...
function automationAllowed(customer, lane) {
    if (customer.optedOut)
        return { allowed: false, reason: "Customer opted out of automated messages" };
    const control = lane === "SALES" ? customer.salesControl : customer.loanControl;
    if (control === "HUMAN_CONTROL")
        return { allowed: false, reason: `${lane} lane is under human control` };
    return { allowed: true };
}
function snapshot(customerId) {
    ...
    return {
        customer,
        trend: (0, intelligence_1.sentimentTrend)(customerId),
