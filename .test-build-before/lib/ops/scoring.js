...
exports.signalsForCustomer = signalsForCustomer;
exports.rescoreCustomer = rescoreCustomer;
exports.bandFor = bandFor;
exports.signalsFromInsight = signalsFromInsight;
...
function computeScore(orgId, signals) {
    ...
    const band = score <= cold ? "COLD" : score <= warm ? "WARM" : score <= hot ? "HOT" : "VERY_HOT";
    return { score, band, contributions };
}
/** Derive the signal set from stored state — no model call required. */
function signalsForCustomer(customerId) {
 ...
 */
function rescoreCustomer(customerId, extra = {}) {
    const db = (0, db_1.read)();
    const customer = db.customers.find((c) => c.id === customerId);
    if (!customer)
        return null;
...
function bandFor(orgId, score) {
    const { cold, warm, hot } = (0, config_1.getConfig)(orgId).scoring.bands;
    return score <= cold ? "COLD" : score <= warm ? "WARM" : score <= hot ? "HOT" : "VERY_HOT";
}
/** Signals implied by a single freshly-extracted insight. */
function signalsFromInsight(i, customer) {
    return {
