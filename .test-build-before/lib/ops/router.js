...
exports.topicForIntent = topicForIntent;
exports.detectLanguage = detectLanguage;
exports.nextStep = nextStep;
exports.withNextStep = withNextStep;
exports.callbackWindow = callbackWindow;
exports.factsSentence = factsSentence;
exports.deterministicReply = deterministicReply;
exports.parseVisitPreference = parseVisitPreference;
exports.matchSlots = matchSlots;
...
function nextStep(lang, mode = "either") {
    const t = {
        en: {
...
function withNextStep(text, lang, mode = "either") {
    const t = text.trim();
    if (/\?\s*$/.test(t))
        return t;
...
function callbackWindow(now = new Date(), timeZone = "Asia/Kolkata") {
...
const T = {
    handoff: {
        en: (who, when) => `of course — I've passed this to ${who}, who will call you ${when}. If there's a time that suits you better, tell me and I'll note it.`,
        hi: (who, when) => `ज़रूर — मैंने यह ${who} को भेज दिया है, वे आपको ${when} कॉल करेंगे। अगर कोई और समय बेहतर हो तो बताइए, मैं नोट कर लूँगा।`,
        hinglish: (who, when) => `bilkul — maine yeh ${who} ko bhej diya hai, woh aapko ${when} call karenge. Agar koi aur time better ho toh bataiye, main note kar lunga.`,
        te: (who, when) => `తప్పకుండా — దీన్ని ${who}కి పంపాను, వారు మీకు ${when} కాల్ చేస్తారు. వేరే సమయం అనుకూలమైతే చెప్పండి, నోట్ చేస్తాను.`,
    },
    callback: {
        en: (when) => `done — I've asked the sales team to call you ${when}. Is there a particular time that works best for you?`,
        hi: (when) => `ठीक है — मैंने सेल्स टीम से कहा है कि वे आपको ${when} कॉल करें। क्या कोई खास समय आपके लिए बेहतर रहेगा?`,
        hinglish: (when) => `theek hai — maine sales team ko bola hai ki woh aapko ${when} call karein. Koi khaas time aapke liye best rahega?`,
        te: (when) => `సరే — సేల్స్ టీమ్ మీకు ${when} కాల్ చేయమని చెప్పాను. మీకు ఏ సమయం బాగా అనుకూలం?`,
    },
    thanks: {
    ...
    when: {
        en: (w) => w,
        hi: (w) => (w.startsWith("within") ? "अगले 2 घंटे में" : "कल सुबह 10 बजे तक"),
        hinglish: (w) => (w.startsWith("within") ? "agle 2 ghante mein" : "kal subah 10 baje tak"),
        te: (w) => (w.startsWith("within") ? "రాబోయే 2 గంటల్లో" : "రేపు ఉదయం 10 గంటలలోపు"),
    },
};
const MAX_FACT_CHARS = 520;
/** Two or three KB answers joined into one readable paragraph. */
function factsSentence(entries, opts = {}) {
    const usable = entries.filter((e) => !(opts.withholdPrices && e.topic === "pricing" && !e.public));
    let out = "";
    for (const e of usable) {
        const a = e.answer.replace(/\s+/g, " ").trim();
        const s = /[.!?]$/.test(a) ? a : `${a}.`;
        if (out.length + s.length > MAX_FACT_CHARS && out)
            break;
        out = out ? `${out} ${s}` : s;
    }
    return out;
 ...
 */
function deterministicReply(intent, ctx) {
...
function parseVisitPreference(text) {
    const t = text.toLowerCase();
    const p = {};
    const wd = t.match(/\b(sun|mon|tue|wed|thu|fri|sat)(day|nesday|rsday|urday|sday)?\b/);
    if (wd)
        p.weekday = WEEKDAYS.indexOf(wd[1]);
    if (/\b(tomorrow|kal|రేపు)\b|कल/.test(t))
        p.dayOffset = 1;
    else if (/\b(today|aaj|ఈరోజు)\b|आज/.test(t))
        p.dayOffset = 0;
    else if (/\bday after\b|parso|परसों/.test(t))
        p.dayOffset = 2;
    if (/\bweekend\b/.test(t))
        p.weekend = true;
    if (/\b(morning|subah|ఉదయం)\b|सुबह/.test(t))
        p.period = "morning";
    else if (/\b(afternoon|dopahar|మధ్యాహ్నం)\b|दोपहर/.test(t))
        p.period = "afternoon";
    else if (/\b(evening|shaam|sham|సాయంత్రం)\b|शाम/.test(t))
        p.period = "evening";
...
function matchSlots(all, pref, timeZone, now = new Date()) {
    ...
    return all.filter((s) => {
        ...
        if (target && lp.ymd !== target)
            return false;
        if (pref.weekday !== undefined && lp.weekday !== pref.weekday)
            return false;
        if (pref.weekend && lp.weekday !== 0 && lp.weekday !== 6)
            return false;
        if (pref.hour !== undefined && Math.abs(lp.hour - pref.hour) > 1)
            return false;
        if (pref.period === "morning" && lp.hour >= 12)
            return false;
        if (pref.period === "afternoon" && (lp.hour < 12 || lp.hour >= 17))
            return false;
        if (pref.period === "evening" && lp.hour < 17)
            return false;
        return true;
    });
}
