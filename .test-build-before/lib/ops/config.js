...
function getConfig(orgId) {
    const existing = (0, db_1.read)().workflowConfigs.find((c) => c.orgId === orgId);
    if (existing)
        return existing;
    const created = defaultConfig(orgId);
    (0, db_1.mutate)((db) => void db.workflowConfigs.push(created));
    return created;
}
...
function updateConfig(orgId, patch) {
    return (0, db_1.mutate)((db) => {
        ...
        const next = { ...base, ...patch, orgId, updatedAt: new Date().toISOString() };
        if (i >= 0)
            db.workflowConfigs[i] = next;
        else
            db.workflowConfigs.push(next);
        return next;
    });
}
