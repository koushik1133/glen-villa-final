...
exports.clearYouTubeCache = clearYouTubeCache;
exports.youtubeChannelRef = youtubeChannelRef;
exports.resolveChannel = resolveChannel;
exports.listUploads = listUploads;
exports.listComments = listComments;
exports.fetchYouTubeSnapshot = fetchYouTubeSnapshot;
exports.fetchYouTubeSnapshotResult = fetchYouTubeSnapshotResult;
exports.fetchYouTubeComments = fetchYouTubeComments;
exports.fetchYouTubeRecentComments = fetchYouTubeRecentComments;
...
async function api(path, params, opts = {}) {
    ...
    if (!key)
        throw new YouTubeError("not_configured", "YouTube is not configured. Set YOUTUBE_API_KEY (or GOOGLE_SHEETS_API_KEY) with the YouTube Data API enabled.");
    ...
    if (!opts.fresh && hit && Date.now() - hit.at < (opts.ttlMs ?? CACHE_TTL_MS))
        return hit.value;
    ...
    if (!res.ok) {
        ...
        if (reason === "commentsDisabled")
            throw new YouTubeError("comments_disabled", "Comments are turned off on this video.", res.status);
        if (res.status === 403)
            throw new YouTubeError("forbidden", message, res.status, reason);
        if (res.status === 404)
            throw new YouTubeError("not_found", message, res.status, reason);
 ...
 */
function youtubeChannelRef(conn) {
...
async function resolveChannel(handle, opts = {}) {
    const h = handle.trim().replace(/^@/, "");
    const params = /^UC[\w-]{20,}$/.test(h)
        ? { id: h }
        : { forHandle: h };
    const raw = await api("channels", { part: "snippet,contentDetails,statistics", ...params }, opts);
    ...
    if (!channel)
        throw new YouTubeError("not_found", `No YouTube channel for handle "${handle}".`);
    return channel;
}
/**
 ...
 */
async function listUploads(channelId, max = 50, opts = {}) {
    ...
    while (out.length < max) {
        ...
        try {
            ...
            if (e instanceof YouTubeError && e.reason === "playlistNotFound")
                return out;
            throw e;
        }
        const ids = (page.items ?? []).map((i) => i.contentDetails?.videoId).filter((x) => Boolean(x));
        if (ids.length === 0)
            break;
        const vids = await api("videos", { part: "snippet,contentDetails,statistics", id: ids.join(","), maxResults: 50 }, opts);
...
async function listComments(videoId, max = 50) {
    const out = [];
    let pageToken;
    while (out.length < max) {
        const page = await api("commentThreads", {
            part: "snippet",
            videoId,
 ...
 */
async function fetchYouTubeSnapshot(handle) {
...
async function fetchYouTubeSnapshotResult(handle, opts = {}) {
    try {
...
async function fetchYouTubeComments(videoId, max = 50) {
    try {
 ...
 */
async function fetchYouTubeRecentComments(videos, videoCount = 10, perVideo = 10) {
    try {
        ...
        videos.slice(0, videoCount).filter((v) => v.comments > 0).map(async (video) => {
            try {
                ...
                if (e instanceof YouTubeError && e.code === "comments_disabled")
                    return { video, threads: [] };
                throw e;
            }
        }));
        return { ok: true, comments: (0, studio_1.mergeRecentComments)(perVideoThreads) };
    }
    catch (e) {
        return toResult(e);
    }
