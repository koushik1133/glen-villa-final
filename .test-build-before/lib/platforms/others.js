...
exports.DRIVER = exports.ALL_CONTENT_ADAPTERS = exports.googleBusiness = exports.x = exports.linkedin = exports.youtube = exports.tiktok = void 0;
const types_1 = require("./types");
Object.defineProperty(exports, "DRIVER", { enumerable: true, get: function () { return types_1.DRIVER; } });
/**
 ...
 */
async function tiktokPost(path, token, body) {
 ...
 */
async function tiktokPublish(req) {
    ...
    try {
        ...
        return { ok: false, error: e.message, retryable: tiktokRetryable(e) };
    }
    /**
    ...
    for (let i = 0; i < TIKTOK_POLL_ATTEMPTS; i++) {
        ...
        if (state === "PUBLISH_COMPLETE") {
            ...
            const permalink = postId && /^@[A-Za-z0-9._]{1,24}$/.test(handle)
...
exports.tiktok = {
    ...
    validate: (req) => (0, types_1.baseValidate)(req, exports.tiktok.capabilities, "TikTok"),
    publish: async (req) => (types_1.DRIVER === "mock" ? (0, types_1.mockPublish)("tiktok", req) : tiktokPublish(req)),
 ...
 */
async function ytPublish(req) {
    const token = req.connection.accessToken;
    if (!token)
        return { ok: false, error: "YouTube connection has no access token.", retryable: false };
    const source = req.mediaUrls[0];
    if (!source)
        return { ok: false, error: "YouTube needs a video file to upload.", retryable: false };
    ...
    try {
        ...
        if (!initRes.ok)
            throw ytError(initRes.status, await ytJson(initRes));
        ...
        if (!put.ok)
            throw ytError(put.status, created);
        ...
        return { ok: false, error: e.message, retryable: ytRetryable(e) };
    }
}
exports.youtube = {
    channel: "youtube",
    ...
    validate: (req) => (0, types_1.baseValidate)(req, exports.youtube.capabilities, "YouTube"),
    publish: async (req) => (types_1.DRIVER === "mock" ? (0, types_1.mockPublish)("youtube", req) : ytPublish(req)),
 ...
 */
function linkedinCreateRetryable(e) {
    const err = e;
 ...
 */
async function linkedinAttempt(step, worthRetrying = linkedinRetryable) {
    let last;
    for (let i = 0; i < LINKEDIN_ATTEMPTS; i++) {
        try {
            ...
            last = e;
            if (i === LINKEDIN_ATTEMPTS - 1 || !worthRetrying(e))
                break;
 ...
 */
async function linkedinCall(path, init) {
 ...
 */
async function linkedinDownload(url) {
 ...
 */
async function linkedinUploadImage(url, owner, token) {
 ...
 */
async function linkedinUploadVideo(url, owner, token) {
    const media = await linkedinDownload(url);
    ...
    for (const part of parts) {
        if (!part.uploadUrl)
            throw new Error("LinkedIn: video upload instruction had no URL");
        ...
        if (!etag)
            throw new Error("LinkedIn: video part upload returned no ETag to finalize with");
        partIds.push(etag);
    }
 ...
 */
async function linkedinPublish(req) {
    const token = req.connection.accessToken;
    if (!token) {
    ...
    try {
        ...
        for (const url of req.mediaUrls) {
            assets.push(await linkedinAttempt(() => isVideo(url) ? linkedinUploadVideo(url, author, token) : linkedinUploadImage(url, author, token)));
        }
        const body = {
            author,
            commentary,
        ...
        if (assets.length === 1)
            body.content = { media: { id: assets[0] } };
        else if (assets.length > 1)
            body.content = { multiImage: { images: assets.map((id) => ({ id })) } };
        const created = await linkedinAttempt(() => linkedinCall("posts", { method: "POST", token, body }), linkedinCreateRetryable);
        ...
        return { ok: false, error: e.message, retryable: linkedinRetryable(e) };
    }
}
exports.linkedin = {
    channel: "linkedin",
    ...
    validate: (req) => (0, types_1.baseValidate)(req, exports.linkedin.capabilities, "LinkedIn"),
    publish: async (req) => (types_1.DRIVER === "mock" ? (0, types_1.mockPublish)("linkedin", req) : linkedinPublish(req)),
...
function xErrorText(body, status) {
 ...
 * never landed.
 */
function xCreateRetryable(e) {
    const err = e;
    return err.status === undefined || err.status === 429;
}
async function xAttempt(step, worthRetrying = xRetryable) {
    let last;
    for (let i = 0; i < X_ATTEMPTS; i++) {
        try {
            ...
            last = e;
            if (i === X_ATTEMPTS - 1 || !worthRetrying(e))
                break;
            await new Promise((r) => setTimeout(r, 1000 * 2 ** i));
        }
    }
...
async function xCall(path, init) {
...
async function xUploadCall(form, token) {
...
async function xUploadStatus(mediaId, token) {
 ...
 */
async function xUploadChunked(bytes, contentType, token) {
    ...
    if (!mediaId)
        throw new Error("X did not return a media id to upload into");
    ...
    for (let i = 0; info && info.state !== "succeeded" && i < X_STATUS_ATTEMPTS; i++) {
        if (info.state === "failed") {
            throw Object.assign(new Error(`X could not process the video: ${info.error?.message ?? "no reason given"}`), { status: 400 });
        }
        await new Promise((r) => setTimeout(r, Math.max(1, info?.check_after_secs ?? 3) * 1000));
        info = (await xUploadStatus(mediaId, token)).processing_info;
    }
 ...
 */
async function xUploadMedia(url, token) {
    ...
    if (!id)
        throw new Error("X did not return a media id for the upload");
    return id;
}
async function xPublish(req) {
    const token = req.connection.accessToken;
    if (!token) {
    ...
    try {
        ...
        if (!id)
            return { ok: false, error: "X accepted the post but returned no tweet id.", retryable: false };
        if (req.firstComment) {
        ...
        return { ok: false, error: e.message, retryable: xRetryable(e) };
    }
}
exports.x = {
    channel: "x",
    ...
    validate: (req) => (0, types_1.baseValidate)(req, exports.x.capabilities, "X"),
    publish: async (req) => (types_1.DRIVER === "mock" ? (0, types_1.mockPublish)("x", req) : xPublish(req)),
    rateLimit: async () => ({ used: 0, quota: 100, windowHours: 24 }),
};
/* -------------------------------------------------------------------------- */
 ...
 */
function gbpId(value, collection) {
 ...
 */
function gbpRetryable(status, canonical) {
    if (status === 429 || status >= 500)
        return true;
    return ["RESOURCE_EXHAUSTED", "UNAVAILABLE", "INTERNAL", "DEADLINE_EXCEEDED", "ABORTED"].includes(canonical ?? "");
}
/**
...
async function gbpPublish(req) {
    ...
    try {
        ...
        if (!dispatched) {
            ...
            return { ok: false, error: `Google Business: ${e.message}`, retryable: true };
        }
        ...
        return {
            ok: false,
            error: `Google Business: the post was sent but the response could not be read (${e.message}). Check the location before posting again — it may already be live.`,
            retryable: false,
        };
    }
}
...
exports.googleBusiness = {
    ...
    validate(req) {
        const errors = (0, types_1.baseValidate)(req, exports.googleBusiness.capabilities, "Google Business");
        if (req.hashtags.length)
            errors.push("Google Business posts ignore hashtags — drop them.");
        return errors;
    },
    publish: async (req) => (types_1.DRIVER === "mock" ? (0, types_1.mockPublish)("google_business", req) : gbpPublish(req)),
    rateLimit: async () => ({ used: 0, quota: 10, windowHours: 24 }),
};
exports.ALL_CONTENT_ADAPTERS = [exports.tiktok, exports.youtube, exports.linkedin, exports.x, exports.googleBusiness];
