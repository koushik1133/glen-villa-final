...
exports.flattenExtracted = flattenExtracted;
exports.detectIntent = detectIntent;
exports.extractedName = extractedName;
exports.toRecord = toRecord;
exports.ingestExecution = ingestExecution;
const db_1 = require("../db");
const ids_1 = require("../ids");
const rules_1 = require("../crm/rules");
...
function isTerminal(status) {
    return status !== null && exports.TERMINAL_STATUSES.has(status.toLowerCase());
}
function outcomeOf(status, durationSec) {
    const s = (status ?? "").toLowerCase();
    if (!isTerminal(s))
        return "in_progress";
    if (s === "completed")
        return durationSec !== null && durationSec > 0 ? "completed" : "no_answer";
    if (s === "no-answer" || s === "busy" || s === "canceled" || s === "cancelled" || s === "stopped")
        return "no_answer";
    return "failed";
}
 ...
 */
function flattenExtracted(input) {
    const out = {};
    if (!input)
        return out;
    const scalar = (v) => typeof v === "string" ? v.trim() || null
    ...
    const leaf = (v) => {
        ...
        if (Array.isArray(v))
            return v.map(scalar).filter(Boolean).join(", ") || null;
        return null;
    };
    for (const [category, fields] of Object.entries(input)) {
        ...
        if (fields && typeof fields === "object" && !Array.isArray(fields)) {
            for (const [field, value] of Object.entries(fields)) {
                const fl = leaf(value);
                if (fl !== null)
                    out[`${category} · ${field}`] = fl.slice(0, 500);
            }
        }
    }
    return out;
...
const INTEREST = /\b(interested|site visit|visit the (site|villa|project)|book(ing)?|schedule|budget|price|pricing|brochure|floor plan|loan|emi|token|when can i|send (me )?details)\b/i;
...
function detectIntent(input) {
    ...
    const callerText = input.turns.length
        ...
        : (input.transcript ?? "");
    if (NEGATIVE.test(callerText))
        return "none";
    if (CALLBACK.test(callerText))
        return "callback";
    if (INTEREST.test(callerText))
        return "interested";
    return "none";
}
/**
...
function extractedName(extracted) {
    for (const [k, v] of Object.entries(extracted)) {
        if (!/\bname\b/i.test(k) || NOT_PERSON_NAME.test(k))
            continue;
        if (v && v.length <= 80 && !/^(unknown|n\/a|none|null)$/i.test(v))
            return v;
    }
    return null;
}
function toRecord(execution, brandId, existing) {
    const now = new Date().toISOString();
    const direction = execution.callType?.toLowerCase().includes("in") ? "inbound"
    ...
    const turns = (execution.turns ?? (0, client_1.turnsFromText)(execution.transcript) ?? []).map((t) => ({
    ...
    const status = locked && !isTerminal(execution.status) ? existing.status
        : execution.status ?? existing?.status ?? "unknown";
    const durationSec = locked && !isTerminal(execution.status) ? existing.durationSec
 ...
 */
function ingestExecution(execution, ctx) {
    const before = (0, db_1.read)().voiceCalls.find((c) => c.executionId === execution.id);
    const record = toRecord(execution, ctx.brandId, before);
    (0, db_1.mutate)((db) => {
        const i = db.voiceCalls.findIndex((c) => c.executionId === record.executionId);
        if (i === -1)
            db.voiceCalls.push(record);
        else
            db.voiceCalls[i] = record;
    });
...
function finalise(record, orgId) {
    const now = new Date().toISOString();
    ...
    if (record.callerPhone) {
        const { customer } = (0, customers_1.upsertCustomer)({
            orgId,
        ...
        if (spoke) {
            const body = record.turns.length
            ...
            (0, db_1.mutate)((db) => {
                ...
                if (db.opsMessages.some((m) => m.externalId === record.executionId))
                    return;
                db.opsMessages.push({
                    id: (0, ids_1.uid)("msg"),
                    orgId,
        ...
        if (wantsLead) {
            const key = (0, customers_1.normalisePhone)(record.callerPhone);
            const existingLead = (0, db_1.read)().leads.find((l) => l.brandId === record.brandId && (0, customers_1.normalisePhone)(l.phone) === key);
            if (existingLead) {
                ...
                lead.score = (0, rules_1.scoreLead)(lead);
                (0, db_1.mutate)((db) => void db.leads.push(lead));
                (0, publisher_1.logActivity)(record.brandId, "crm", `New lead from voice agent: ${lead.name}`, "voice-agent");
                leadId = lead.id;
                leadCreated = true;
            }
            (0, db_1.mutate)((db) => {
    ...
    const finalised = { ...record, customerId, leadId, leadCreated, finalisedAt: now, updatedAt: now };
    (0, db_1.mutate)((db) => {
        const i = db.voiceCalls.findIndex((c) => c.executionId === record.executionId);
        if (i !== -1)
            db.voiceCalls[i] = finalised;
    });
    return finalised;
}
