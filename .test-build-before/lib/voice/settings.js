...
function defaultConfig(brandId, businessName) {
    return {
        brandId,
        businessName,
...
function getConfig(brandId, businessName) {
    return (0, db_1.read)().voiceAgentConfigs.find((c) => c.brandId === brandId) ?? defaultConfig(brandId, businessName);
}
function text(v, max) {
...
function validateConfig(body, ctx) {
    ...
    if (!greeting)
        return { ok: false, error: "A greeting is required — it is the first thing callers hear." };
    const langsRaw = Array.isArray(b.languages) ? b.languages : [];
    const languages = types_1.VOICE_LANGUAGES.filter((l) => langsRaw.includes(l));
    if (!languages.length)
...
function saveConfig(config) {
    (0, db_1.mutate)((db) => {
        ...
        if (i === -1)
            db.voiceAgentConfigs.push(config);
        else
            db.voiceAgentConfigs[i] = config;
    });
}
/**
...
async function syncConfig(config) {
    ...
    const agentId = (0, client_1.configuredAgentId)();
    if (!agentId) {
        return {
            synced: false,
    ...
    const res = await (0, client_1.updateAgent)(agentId, {
...
function recordSync(brandId, result) {
    (0, db_1.mutate)((db) => {
        const c = db.voiceAgentConfigs.find((x) => x.brandId === brandId);
        if (c)
            c.lastSync = { at: new Date().toISOString(), ok: result.synced, message: result.message };
    });
}
